...
@section('content')
    <div class="w-full max-w-6xl mx-auto bg-white rounded-lg shadow-lg overflow-x-auto my-20 text-md">
        ...
        <div id="orderDetails" class="p-6">
            <p class="mb-2 text-gray-700">Nama: <span id="agenName"
                    class="font-semibold">{{ $pesanMasukDistributor['nama_agen'] }}</span></p>
            <p class="mb-2 text-gray-700">Tanggal Pemesanan: <span id="orderDate"
                    class="font-semibold">{{ $pesanMasukDistributor['tanggal'] }}</span></p>
            ...
            <table class="w-full mb-4">
                ...
                <tbody>
                    @foreach ($pesanMasukDistributor['item_nota'] as $item)
                        <tr class="border-b">
                ...
                <tfoot class="bg-gray-100">
                    <tr>
                        ...
                        <td class="p-2 text-right font-semibold" id="total-amount">Rp
                            {{ number_format($pesanMasukDistributor['total_harga'], 0, ',', '.') }}</td>
                    </tr>
                </tfoot>
            </table>
            ...
            <p class="mb-4 text-gray-700">Status Pemesanan:
                <span id="orderStatus"
                    class="font-bold {{ $pesanMasukDistributor['status'] == 1 ? 'text-green-600' : ($pesanMasukDistributor['status'] == 2 ? 'text-red-600' : 'text-orange-600') }}">
                    {{ $pesanMasukDistributor['status'] == 1 ? 'Selesai' : ($pesanMasukDistributor['status'] == 2 ? 'Ditolak' : 'Diproses') }}
                </span>
            </p>


            <!-- Edit Button (only show when status is 0) -->
            @if ($pesanMasukDistributor['status'] == 0)
        ...
        <div id="editSection" class="p-6 hidden bg-white shadow-lg rounded-lg">
            ...
            <!-- Status Pemesanan -->
            <form id="statusForm" action="{{ route('updateStatusPesananDistributor', $pesanMasukDistributor['id_order']) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
                    ...
                    <select id="orderStatusDropdown" name="status"
                        ...
                        <option value="0" {{ $pesanMasukDistributor['status'] == 0 ? 'selected' : '' }}>Diproses</option>
                        <option value="1" {{ $pesanMasukDistributor['status'] == 1 ? 'selected' : '' }}>Selesai</option>
                        <option value="2" {{ $pesanMasukDistributor['status'] == 2 ? 'selected' : '' }}>Ditolak</option>
                    </select>
                </div>
                <div class="flex justify-end mt-6">
    ...
    <div id="gambar" tabindex="-1" aria-hidden="true"
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 max-w-lg w-full">
            ...
            @if ($pesanMasukDistributor['gambar'])
                <img src="{{ asset('storage/' . $pesanMasukDistributor['gambar']) }}" alt="Bukti Pembayaran"
