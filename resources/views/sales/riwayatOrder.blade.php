            ...
            <table class="w-full border-separate border-spacing-0">
                ...
                <tbody class="bg-white text-sm">
                    ...
                    <tr class="border-b border-gray-200">
                        ...
                        <td class="p-2">
                            @if(empty($orderSale->bukti_transfer))
                            <!-- Jika bukti_transfer kosong, tampilkan tombol Bayar -->
                            <button onclick="window.location.href='{{ route('bayar_nota', $orderSale->id_order) }}'" class="bg-yellow-600 text-white font-bold py-1 px-3 rounded hover:bg-yellow-700 focus:outline-none focus:ring-2 focus:ring-yellow-500 text-xs">
                                Bayar
                            </button>
                            @elseif($orderSale->status_pemesanan == 1)
                            <!-- Jika bukti_transfer ada dan status_pemesanan adalah 1, tampilkan tombol Tersedia -->
                            <button onclick="window.location.href='{{ route('nota_sales', $orderSale->id_order) }}'" class="bg-green-600 text-white font-bold py-1 px-3 rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 text-xs">
                                Tersedia
                            </button>
                            @else
