                                    ...
                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                        ...
                                        <div class="p-6 text-center">
                                            ...
                                            <form action="{{ route('kunjunganToko.update', $visit->id_kunjungan_toko) }}"
                                                ...
                                                <div class="text-left mb-4">
                                                    <label for="sisa_produk" class="block text-sm font-medium text-gray-900">Produk
                                                        Terjual</label>
                                                ...
                                                <div class="text-left mb-4">
                                                    <label for="gambar-{{ $visit->id_kunjungan_toko }}" id="file-input-label"
                                                        class="block text-sm font-medium text-gray-900">Dokumentasi</label>
                                                    <p id="file-error-{{ $visit->id_kunjungan_toko }}"
                                                        class="mt-1 text-sm text-red-500" style="display:none;">Gambar tidak boleh
                                                        berukuran lebih dari 1 MB.</p>
                                                    <input type="file" id="gambar-{{ $visit->id_kunjungan_toko }}" name="gambar"
                                                        accept=".jpg, .jpeg, .png"
            ...
            <div class="p-6 text-center">
                ...
                <form action="" enctype="multipart/form-data" method="POST" class="space-y-4">
                    ...
                    <div class="text-left">
                        ...
                        <p id="file-error" class="mt-1 text-sm text-red-500" style="display:none;">Gambar tidak boleh
                            berukuran lebih dari 1 MB.</p>
...
@if ($kunjunganToko->total() > 5)
    <div class="flex flex-col items-center my-6">
        ...
        <span class="text-sm text-gray-700 dark:text-gray-400">
            Menampilkan <span class="font-semibold text-gray-900 dark:text-white">{{ $kunjunganToko->firstItem() }}</span>
            sampai
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        document.querySelectorAll('input[type="file"]').forEach(input => {
            input.addEventListener('change', function () {
                ...
                const fileErrorId = this.id.includes('-') ? 'file-error-' + this.id.split('-')[1] : 'file-error';
        ...
        document.querySelectorAll('form').forEach(form => {
            form.addEventListener('submit', function (event) {
                ...
                fileInputs.forEach(fileInput => {
                    const fileErrorId = fileInput.id.includes('-') ? 'file-error-' + fileInput.id.split('-')[1] : 'file-error';
                    const fileError = document.getElementById(fileErrorId);
