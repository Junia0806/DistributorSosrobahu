...
@section('content')
    <section class="container mx-auto p-6 relative my-20">
        <section class="p-6 border rounded-lg bg-white shadow-lg">
            ...
            <div id="account-info" class="mb-6">
                ...
                <div class="mb-6 p-4 bg-blue-100 border border-blue-400 rounded-lg flex items-center space-x-2">
                    ...
                    <p class="text-gray-700">
                        Berikut adalah informasi rekening <strong>Official CV. Santoso Jaya Tembakau</strong>. Semua transaksi dari Distributor akan menggunakan nomor rekening ini.
                    </p>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Nama Bank</label>
                    <p class="text-gray-800">{{ $userPabrik['nama_bank'] }}</p>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Nomor Rekening</label>
                    <p class="text-gray-800">{{ $userPabrik['no_rek'] }}</p>
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 font-medium mb-2">Atas Nama</label>
                    <p class="text-gray-800">{{ $userPabrik['nama_pabrik'] }}</p>
                </div>
            ...
            <!-- Form Rekening Bank -->
            <form action="{{ route('rekeningBankPabrik.update') }}" id="account-form" class="hidden" method="POST" >
                @csrf
                @method('PUT')
                ...
                <div class="mb-4">
                    ...
                    <select id="bank_name" name="bank_name"
                        ...
                        <option value="Bank Mandiri" {{ $userPabrik['nama_bank'] == 'Bank Mandiri' ? 'selected' : '' }}>Bank Mandiri</option>
                        <option value="Bank BCA" {{ $userPabrik['nama_bank'] == 'Bank BCA' ? 'selected' : '' }}>Bank BCA</option>
                        <option value="Bank BNI" {{ $userPabrik['nama_bank'] == 'Bank BNI' ? 'selected' : '' }}>Bank BNI</option>
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" id="account_number" name="account_number"
                        class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Masukkan Nomor Rekening" value="{{ $userPabrik['no_rek'] }}" required>
                </div>
                ...
                <div class="mb-6">
                    ...
                    <input type="text" id="account_holder" name="account_holder"
                        class="w-full p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                        placeholder="Masukkan Nama Pemilik Rekening" value="{{ $userPabrik['nama_pabrik'] }}" required>
                </div>
