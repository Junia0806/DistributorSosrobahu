    ...
    <section class="container mx-auto p-6 my-20">
                ...
                <table class="w-full border-separate border-spacing-0">
                    ...
                    <tbody class="bg-white text-sm">
                        ...
                        <tr class="border-b border-gray-200">
                            ...
                            <td class="p-2 {{ $orderAgen->status_pemesanan == 1 ? 'text-green-600' : 'text-red-600' }} font-semibold">
                            ...
                            <td class="p-2">
                                ...
                                <button onclick="window.location.href='{{ route('notaAgen', $orderAgen->id_order) }}'" class="bg-green-600 text-white font-bold py-1 px-3 rounded hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 text-xs">
                                    Tersedia
                                </button>
                                @else
        ...
        <div class="flex flex-col items-center my-6">
            ...
            <span class="text-sm text-gray-700 dark:text-gray-400">
                Menampilkan <span class="font-semibold text-gray-900 dark:text-white">{{ $orderAgens->firstItem() }}</span> sampai <span class="font-semibold text-gray-900 dark:text-white">{{ $orderAgens->lastItem() }}</span> dari <span class="font-semibold text-gray-900 dark:text-white">{{ $orderAgens->total() }}</span> transaksi
            </span>
            <!-- Buttons -->
            <div class="inline-flex mt-2 xs:mt-0">
                <!-- Previous Button -->
                <button
                    {{ $orderAgens->onFirstPage() ? 'disabled' : '' }}
                ...
                <!-- Next Button -->
                <button
                    {{ !$orderAgens->hasMorePages() ? 'disabled' : '' }}
